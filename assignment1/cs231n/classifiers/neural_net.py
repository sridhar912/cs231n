...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    z2_class_sums = np.sum(z2_exp, axis=1)
    a2 = z2_exp / z2_class_sums[:, np.newaxis]
    rows = np.arange(N)  # N is the number of input images
    cols = y[:, np.newaxis]
    a2_correct = a2[rows[:, np.newaxis], cols]
    a2_loss = -1 * np.log(a2_correct)  # See lecture3 page 29
